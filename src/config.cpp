// =============================================================================
//  config.cpp  -  load tunables from PadWithin.ini next to the DLL.
//
//  Uses the Win32 GetPrivateProfile* API so the ini format is the familiar
//  [Section] key=value. The file is looked up by absolute path next to our
//  own module, so it works regardless of the game's working directory.
// =============================================================================
...
static std::string IniPath(const char* iniName)
{
    ...
    return iniName;   // fallback: relative to cwd
}
...
void Config_Load(const char* iniName)
{
    ...
    {
        // VID/PID may be written as hex (0x054c) or decimal; parse via strtol.
        char buf[32];
